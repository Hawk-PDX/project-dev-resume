...
def test_get_projects_empty_db(client):
    """Test GET /projects/ returns sample data when DB is empty."""
    response = client.get('/')
...
def test_create_project(client):
    ...
    response = client.post('/', json=project_data)
...
def test_update_project(client):
    ...
    response = client.put(f'/{project_id}', json=update_data)
    assert response.status_code == 200
    data = json.loads(response.data)
...
def test_get_featured_projects(client):
    ...
    client.post('/', json={'title': 'Featured 1', 'featured': True})
    client.post('/', json={'title': 'Non-featured', 'featured': False})
    client.post('/', json={'title': 'Featured 2', 'featured': True})
...
def test_update_nonexistent_project(client):
    ...
    response = client.put('/999', json={'title': 'New Title'})
    assert response.status_code == 404
    data = json.loads(response.data)
    assert data['error'] == 'Project not found'
