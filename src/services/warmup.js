...
export const warmupBackend = async () => {
  if (warmupInProgress || warmupCompleted) {
    console.log('Backend warmup already in progress or completed');
    return;
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.warn('Backend warmup returned non-200 status:', response.status);
    }
  } catch (error) {
    if (error.name === 'AbortError') {
      console.warn('Backend warmup timed out after 30 seconds');
    } else {
  ...
  const preloadPromises = endpoints.map(async (endpoint) => {
    try {
      ...
      if (response.ok) {
        ...
        console.warn(`Failed to preload ${endpoint}:`, response.status);
        return null;
      }
    } catch (error) {
